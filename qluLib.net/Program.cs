...
while (true)
{
    ...
    switch (now.Hour)
    {
        ...
        case 22 when now is { Minute: 00, Second: 0 }:
        {
            ...
            if (mailNotify)
            {
                ...
                {
                    mailData.Subject = $"{task.Result.Item1}-{task.Result.Item2}";
                    mailData.Body = task.Result.Item2 ? $"[{task.Result.Item1}] 预约成功" : $"[{task.Result.Item1}] 预约失败";
    ...
    if(now.Hour != pastHour)
    {
        ...
        {
            mailData.Subject = "qluLib.net";
            mailData.Subject = "预约通知服务开启";
